...
export class HeaderComponent implements OnInit {
    @ViewChild('modalEmpresaInfo') modal: NgxCustomModalComponent | undefined;
    empresaInfoStore: EmpresaInfoStore | null | undefined;
    sucursalIdSelected: string = '';
    codigoActEcoSelected: string = '';
    store: any;
    search = false;
    notifications = [
        {
    ...
    async initStore() {
        ...
        await this.loadEmpresaInformation();

        if (this.sucursalIdSelected === '' || this.codigoActEcoSelected === '') {
            this.loadEmpresaInformation();
            this.modal?.open();
        }
    }
    ...
    onCheckInfoSelected() {
        this.modal?.close();
        this.alert.setStoreFacturacionConfig(this.sucursalIdSelected, this.codigoActEcoSelected);
    }
    ...
    onCancel() {
        if (this.sucursalIdSelected && this.codigoActEcoSelected) {
            ...
            console.warn('Debe seleccionar sucursal y actividad económica antes de cancelar');
        }
    }

    async loadEmpresaInformation() {
        await this.alert.onLoadEmpresaInfo();
        await this.alert.onLoadActEconomicaInfo();
        await this.alert.onLoadCatalogosInfo();
        this.alert.getStoreEmpresaInfo().then((res) => {
            this.empresaInfoStore = res;
        });
        this.alert.getStoreFacturacionConfig()?.then((res) => {
            this.sucursalIdSelected = res?.sucursalIdSelected || '';
            this.codigoActEcoSelected = res?.codActEcoSelected || '';
        });
    }
